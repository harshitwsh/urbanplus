...
import { Play, Pause, ShieldCheck, Clock, Layers, Sparkles } from 'lucide-react';
...
export const Header: React.FC = () => {
  ...
  return (
    <>
      <header className="h-16 bg-[#FFFFFF] border-b border-[#E2E8F0] px-4 md:px-6 flex items-center justify-between z-30 select-none font-sans shrink-0">
        {/* Left Title & Status */}
          ...
          >
            ...
            <div>
              <h1 className="font-bold text-sm text-[#172033] tracking-tight flex items-center space-x-2">
                ...
                <span className="text-xs text-[#2563EB] font-mono font-semibold uppercase">
                  {activeTab.replace('_', ' ')}
                </span>
              </h1>
              <p className="text-[10px] text-[#64748B] font-mono">AI-Powered Mobile Urban Intelligence • BEL / SIH26124</p>
            </div>
          </div>

          <div className="hidden lg:flex items-center space-x-1.5 px-2.5 py-0.5 bg-[#ECFDF5] border border-[#A7F3D0] rounded-md text-[11px] font-mono text-[#059669] font-semibold">
        ...
        <div className="flex items-center space-x-3">
          {/* Clock */}
          <div className="hidden sm:flex items-center space-x-1 text-xs font-mono text-[#64748B] px-2.5 py-1 bg-[#F8FAFC] border border-[#E2E8F0] rounded-md">
          ...
          <button
            ...
            className="px-3 py-1.5 bg-[#FFFFFF] hover:bg-[#F8FAFC] text-[#526174] text-xs font-semibold rounded-md border border-[#CBD5E1] transition flex items-center space-x-1"
          >
          ...
          <button
            ...
            className="px-4 py-1.5 bg-[#2563EB] hover:bg-blue-700 text-white text-xs font-semibold rounded-md shadow-sm transition flex items-center space-x-1.5"
          >
            {isDemoRunning ? <Pause className="w-3.5 h-3.5 fill-current" /> : <Play className="w-3.5 h-3.5 fill-current" />}
            <span>{isDemoRunning ? 'Pause Demo' : '▶ RUN LIVE DEMO'}</span>
          </button>
        </div>
      </header>
